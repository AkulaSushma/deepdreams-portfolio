...
document.addEventListener('DOMContentLoaded', () => {
    // Each initialiser is isolated: these are independent decorative features,
    // and a throw in one used to abort the whole handler — which silently killed
    // every initialiser after it, including the demo modal.
    [
...
function initTraceHeart() {
    ...
    // Recomputed on every resize, so the path always matches the current canvas.
    // Declared before setupCanvas() runs — drawHeart() reads it.
    let heartPoints = [];
    ...
    function setupCanvas() {
        ...
        canvas.width = rect.width;
        canvas.height = rect.height;
        heartPoints = getHeartPoints(canvas.width / 2, canvas.height / 2 + 10, Math.min(canvas.width, canvas.height) * 0.35);
        drawHeart();
    }
